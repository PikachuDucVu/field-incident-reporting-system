...
export function NotificationProvider({ children }: NotificationProviderProps) {
  ...
  const realtimeEnabled = Boolean(token && user);
  ...
  useEffect(() => {
    ...
    socket.on('notification:report-status-updated', addNotification);
    socket.on('connect_error', (error) => {
      console.error('Socket connection failed:', error.message);
    });

    return () => {
      socket.off('notification:report-status-updated', addNotification);
      socket.disconnect();
    };
  ...
  return (
    <NotificationContext.Provider value={value}>
      {children}
      {visibleToast && (
        <div className="fixed left-4 right-4 top-4 z-50 rounded-lg border border-blue-100 bg-white shadow-lg sm:left-auto sm:w-full sm:max-w-sm">
